...
public class DartDao {
	...
	public void insertdb(ArrayList<DartVo> db) {
        ...
        String result =null;
        GetPrintStackTrace getst = new GetPrintStackTrace();
		...
		for(int i =0; i < db.size(); i++) {
	        ...
	        try{
	            ...
	            errorStr = getst.getPrintStackTrace(e);
	            System.out.println("num : "+i+" /다운로드 불가 rcept_no : "+errorStr.substring(136, 150));
	            errRcept.add(errorStr.substring(136, 150));
	            try {
	                if (is != null){
	                    is.close();
	                }
